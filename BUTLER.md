...
| Tab | 职责 |
|-----|------|
| Tours | 以客户为单位管理所有 Tours，查看每个 Tour 的当前状态 |
| Schedule | 以时间为单位查看已确认的看房行程，月历视图，点击日期显示当天行程 |
| + | 居中的主操作按钮，点击新建 Search |
| Client | 客户管理，可以管理客户的基本信息和需求 |
| Inbox | 处理所有通知和决策卡（AI 无法自动处理的例外情况） |


**Tours 是默认首页。** 中介打开 App 直接看到所有 Plan 的状态列表：

```
[ 张先生 — Tampines 3BR ]      🟡 协调中
[ 李小姐 — Bishan HDB ]        🟢 行程已确认
[ 王先生 — Queenstown Condo ]   ⚪ 搜索中
[ 陈太太 — Bedok 4BR ]         🔴 需要处理
```

当某个 Plan 有决策卡需要处理时，Plan 显示"🔴 需要处理"状态，同时 Inbox tab 显示红点提醒。中介在 Inbox 里处理决策，Plan 状态自动更新。搜索功能的入口在 Plan 内部，不作为独立 tab。
...
创建 Plan 只需要填写最基本的信息，中介可以立即进入搜索：

| 字段 | 说明 |
|------|------|
| 客户姓名 | 买家或租客姓名 |
| 客户 WhatsApp 号码 | 用于后续发送确认链接和最终行程 |
| 类型 | 租房 / 买房 |
| 备注 | 任何需要记录的额外信息（选填） |

### 4.2 租房 / 买房档案（选填，可随时补充）

客户档案**不是进入搜索的必要步骤**。中介可以先搜房，之后任何时候回到 Plan 里补充档案信息。

档案内容会在协调阶段由 AI 自动附在发给对方中介的消息里。若协调开始时档案尚未填写，AI 会提醒中介补充，或在没有档案的情况下跳过介绍租客背景这一步，直接进入时间协调。
...
Plan 创建完成后，状态为 `SEARCHING`，中介立即进入搜索步骤。Plan 会持续追踪整个流程的状态变化：

```
SEARCHING → SHORTLISTED → PENDING_BUYER → PLANNING → COORDINATING → CONFIRMED → COMPLETED
```

所有搜索记录、短名单、时间段选择、协调记录、最终行程都归属于这个 Plan，中介可以随时回来查看进度或补充客户档案。
...
左边：买家感兴趣的房源（未分配列表）
右边：地理聚类分组，一个地区一列
      例：Tampines 区 ｜ Bedok 区 ｜ Queenstown 区
```
...
- 直接接受建议分组，点击"开始协调"
- 拖动某个房源到不同的地区分组调整
- 把某个房源拖回左边"未分配"，暂时不纳入这次行程

### 7.2 触发协调

中介对分组满意后，点击"**开始协调**"按钮，Scheduling Agent Skill 正式启动。这是唯一的触发点，中介在此之前可以随意调整而不产生任何对外通信。
