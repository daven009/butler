...
export default function Chat() {
  const nav = useAppNav();
  const { activeTour, activeThreadId, threads, refreshThreads } = useButler();
  const thread = threads.find((item) => item.id === activeThreadId) || threads[0] || null;
  const [takenOver, setTakenOver] = useState(thread?.ownership === 'HUMAN');
  ...
  useEffect(() => {
    setTakenOver(thread?.ownership === 'HUMAN');
  }, [thread]);

  useEffect(() => {
    if (!activeTour?.id || !thread?.id) {
      setMessages([]);
      return;
    }

    toursApi.getThreadMessages(activeTour.id, thread.id)
      .then((response) => setMessages(response.messages || []))
      .catch(() => setMessages([]));
  }, [activeTour, thread]);

  async function handleTakeOver() {
    if (!activeTour?.id || !thread?.id) return;
    await toursApi.updateThreadOwnership(activeTour.id, thread.id, 'HUMAN');
    setTakenOver(true);
    await refreshThreads();
  }

  async function handleSend() {
    if (!draft.trim() || !activeTour?.id || !thread?.id) return;
    const response = await toursApi.sendThreadMessage(activeTour.id, thread.id, draft.trim());
    ...
    <div className="screen screen--bg">
      <header className="screen-header screen-header--glass">
        <div className="screen-header-row">
          ...
          <Avatar label={thread?.avatar || 'A'} color="#E07A5F" size={36} />
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 600, fontSize: 14.5 }}>{thread?.agent || 'Thread'}</div>
            <div style={{ fontSize: 11.5, color: AB.gray }}>{thread?.unit || 'Listing'} · {thread?.time || 'Pending'}</div>
          </div>
      ...
      <div className="screen-footer" style={{ background: AB.white, borderTop: `1px solid ${AB.border}` }}>
        {!takenOver ? (
          <div style={{ padding: '12px 16px 16px' }}>
            <button onClick={handleTakeOver} style={{ width: '100%', padding: '13px', border: `1.5px solid ${AB.ink}`, borderRadius: 12, background: '#fff', fontSize: 14, fontWeight: 600, color: AB.ink, display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ padding: '10px 12px 14px', display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <button onClick={handleSend} style={{ width: 40, height: 40, borderRadius: '50%', border: 0, background: AB.rausch, display: 'grid', placeItems: 'center', padding: 0 }}>
