...
export default function DecisionCard() {
  const nav = useAppNav();
  const { activeTour, exceptions, refreshApp, refreshExceptions } = useButler();
  const [resolved, setResolved] = useState(null);
  const exception = exceptions.find((item) => !item.resolved) || exceptions[0] || null;

  async function resolve(action, key) {
    if (!activeTour?.id || !exception?.id) return;
    await toursApi.resolveException(activeTour.id, exception.id, action);
    setResolved(key);
    await Promise.all([refreshApp(), refreshExceptions()]);
  }
      ...
      <div className="screen-body">
        <div style={{ padding: '18px 20px 24px' }}>
          <div style={{ background: AB.white, borderRadius: 18, padding: '20px', border: `1px solid ${AB.border}`, boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
            ...
            <div style={{ fontFamily: '"Playfair Display", Georgia, serif', fontSize: 22, fontWeight: 600, letterSpacing: -0.3 }}>{exception?.title || 'No active exception'}</div>
            <div style={{ fontSize: 13, color: AB.gray, marginTop: 4 }}>{activeTour?.buyerName || 'Buyer'} · {exception?.owner || 'Butler'}</div>

            <div style={{ marginTop: 16, padding: '14px', background: AB.bg, borderRadius: 12, fontSize: 13.5, lineHeight: 1.5 }}>
              {exception?.detail || 'No manual decision is required right now.'}
            </div>

            {!resolved && exception && (
              <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
                <button onClick={() => resolve('SQUEEZE_IN', 'squeeze')} style={btnPrimary}>
                ...
                <button onClick={() => resolve('REPROPOSE', 'repropose')} style={btnOutline}>Propose new times</button>
                <button onClick={() => resolve('DROP_LISTING', 'drop')} style={{ ...btnOutline, color: AB.gray }}>Drop this unit</button>
              </div>
            )}
