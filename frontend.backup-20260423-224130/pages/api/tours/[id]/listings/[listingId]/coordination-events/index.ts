...
import { validateCreateCoordinationEventInput } from '../../../../../../../lib/api/validation';
import {
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    const coordinationEvents = listCoordinationEvents(tourId, listingId);
    if (!coordinationEvents) return missingTourOrListing(res, tourId, listingId);
  ...
  if (req.method === 'POST') {
    ...
    if (!validation.ok || !validation.value) {
      return sendError(
        ...
        400,
        'VALIDATION_FAILED',
        'Coordination event input is invalid',
        validation.fields
      );
    }

    const result = addCoordinationEvent(tourId, listingId, validation.value);
