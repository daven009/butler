  ...
  const tourItems = useMemo(
    () =>
      workspaceTours.map((tour) => ({
        id: tour.id,
        title: tour.title,
        meta: `${tour.targetDate} · ${tourListingCounts[tour.id] ?? (tour.id === activeTour?.id ? listingItems.length : 0)} listings`,
  ...
  const startScheduling = async () => {
    ...
    try {
      ...
      try {
        ...
        if (seeded.length) setActiveTourListings(activeTour.id, seeded)
      ...
      try {
        ...
        if (updated.status === 'completed' || updated.status === 'failed') {
          ...
          if (updated.status === 'completed' && activeTour) {
            ...
            // Open a chat session in the background, but route preview is
            // the primary completion state: scheduling automatically leads
            // to the generated viewing route.
            try {
...
function PlanWorkspace({
  ...
  const [contentMode, setContentMode] = useState<ContentMode>("listings")
...
function AIAssistantWorkspace({
  ...
  const [listingBriefDrafts, setListingBriefDrafts] = useState<Record<string, string>>({})
  const selectedListing = listings.find((listing) => listing.id === selectedListingId) ?? listings[0] ?? null
  const started = schedulingStarted || schedulingRunning || Boolean(schedulingRun) || Boolean(chatSessionId)
  const needsSingleListingBrief = started && selectedListing?.status === "imported"
  const selectedListingBrief = selectedListing ? listingBriefDrafts[selectedListing.id] ?? "" : ""
  const setSelectedListingBrief = (value: string) => {
    if (!selectedListing) return
    setListingBriefDrafts((drafts) => ({ ...drafts, [selectedListing.id]: value }))
  }

  return (
    <div className={`mt-5 grid min-h-0 flex-1 gap-3 overflow-hidden ${started ? "xl:grid-cols-[280px_minmax(0,1.25fr)_minmax(280px,0.8fr)]" : "xl:grid-cols-[280px_minmax(0,1fr)]"}`}>
      <ListingActivityRail
        listings={listings}
        selectedListingId={selectedListing?.id ?? null}
        schedulingStarted={schedulingStarted}
      ...
      {!started ? (
        ...
        <>
          {needsSingleListingBrief ? (
            <SingleListingScheduleBrief
              listing={selectedListing}
              draft={selectedListingBrief}
              setDraft={setSelectedListingBrief}
              schedulingRunning={schedulingRunning}
              onStartScheduling={onStartScheduling}
            />
          ) : (
    ...
    !hasContact
      ...
      : listing.status === "confirmed"
        ...
        : listing.status === "needs-attention"
          ? { label: "需要处理", detail: listing.attentionReason ?? "对方时间与线路不匹配", tone: "bg-[#fff5f7] text-[#c13515]" }
      ...
      <div className="flex h-full min-h-0 flex-col p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <h2 className="mt-1 text-2xl font-bold tracking-[-0.3px]">告诉 AI 这次排期要怎么做</h2>
            <p className="mt-2 text-sm leading-6 text-[#6a6a6a]">{stageCopy.description}</p>
          </div>
        </div>
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          <p className="text-xs font-semibold text-[#717171]">点击后直接开始排期，不再弹出二次确认。</p>
          <button
            type="button"
          ...
          >
            开始 AI 排期 <Send className="size-4" />
          </button>
        </div>
      </div>
    </section>
...
function SingleListingScheduleBrief({
  listing,
  draft,
  setDraft,
  schedulingRunning,
  ...
  const canStart = Boolean(listing.coAgent.phone) && !schedulingRunning
    ...
    <section className="min-h-0 overflow-hidden rounded-[6px] border border-[#eeeeee] bg-white shadow-sm">
      <div className="flex h-full min-h-0 flex-col p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <p className="text-xs font-bold uppercase tracking-[0.12em] text-[#ff385c]">AI Schedule Brief</p>
            <h2 className="mt-1 text-2xl font-bold tracking-[-0.3px]">为这个新房源补充排期要求</h2>
            <p className="mt-2 text-sm leading-6 text-[#6a6a6a]">
              {listing.condo} 是这条线路开始排期后新增的房源。你可以单独说明这个房源的优先级、可接受时间和处理方式。
            </p>
          </div>
        </div>

        <div className="mt-5 grid gap-2 sm:grid-cols-3">
          <AICheckRow ok label={listing.area} />
          <AICheckRow ok={Boolean(listing.coAgent.phone)} label={listing.coAgent.phone ? "中介联系方式" : "缺少联系方式"} />
          <AICheckRow ok={listing.status === "imported"} label="待加入 AI 排期" />
        </div>
        ...
        <div className="mt-4 min-h-0 flex-1">
          <textarea
            value={draft}
            onChange={(event) => setDraft(event.target.value)}
            placeholder={`例如：${listing.condo} 如果对方中介今天下午 4 点后有空，就插入现有路线；如果会影响已确认房源，先不要改动。`}
            className="h-full min-h-[220px] w-full resize-none rounded-[6px] border border-[#dddddd] bg-[#fafafa] p-4 text-sm font-medium leading-6 text-[#222222] outline-none transition-colors focus:border-[#222222] focus:bg-white"
          />
        </div>

        <div className="mt-4 flex items-center justify-between gap-3">
          <p className="text-xs font-semibold text-[#717171]">AI 会基于这个房源重新尝试协调，并尽量保留已确认安排。</p>
          <button
            type="button"
            onClick={onStartScheduling}
            disabled={!canStart}
            className="inline-flex shrink-0 cursor-pointer items-center justify-center gap-2 rounded-[6px] bg-[#ff385c] px-6 py-3 text-sm font-bold text-white shadow-[0_10px_24px_rgba(255,56,92,0.24)] transition-colors hover:bg-[#e00b41] disabled:cursor-not-allowed disabled:bg-[#dddddd] disabled:text-[#717171] disabled:shadow-none focus:outline-none focus-visible:ring-2 focus-visible:ring-[#222222] focus-visible:ring-offset-2"
          >
            让 AI 安排这个房源 <Send className="size-4" />
          </button>
        </div>
      </div>
    </section>
...
function SimulatedCoAgentThread({ listing }: { listing: Listing | null }) {
  if (!listing) return <p className="text-sm text-[#717171]">选择房源后显示对话。</p>
  const confirmed = listing.status === "confirmed"
  const issue = listing.status === "needs-attention" || !listing.coAgent.phone
  return (
    <div className="space-y-3">
      <CoAgentMessageBubble side="ai" text={`Hi ${listing.coAgent.name || "there"}, 想约 ${listing.condo} 的 viewing，请问今天有没有可看的时间？`} />
      {listing.coAgent.phone ? (
        <CoAgentMessageBubble side="agent" text={confirmed ? `可以，${listing.suggestedTime ?? "建议时间"} 可以安排。` : issue ? "这个时间不太合适，需要换一个 slot。" : "收到，我看一下 availability。"} />
      ) : (
        <CoAgentMessageBubble side="agent" text="无法发送：缺少对方中介 WhatsApp。" muted />
      )}
      <CoAgentMessageBubble side="ai" text={confirmed ? "已记录确认时间，并同步到线路路线。" : "AI 正在根据客户时间窗和其他房源重新计算可行时间。"} />
    </div>
  )
}
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl rounded-[6px] p-0">
        ...
        <DialogFooter className="border-t border-[#eeeeee] p-6 pt-4">
          <DialogClose className="cursor-pointer rounded-[6px] border border-[#dddddd] px-4 py-2.5 text-sm font-bold hover:border-[#222222]">取消</DialogClose>
          <button onClick={submitTour} className="cursor-pointer rounded-[6px] bg-[#ff385c] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#e00b41] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#222222] focus-visible:ring-offset-2">创建线路</button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
