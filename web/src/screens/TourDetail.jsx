...
import { AB, LISTINGS, THREADS } from '../data';
...
function getDefaultListings(shortlistedIds = []) {
  ...
  return THREADS.map((thread) => LISTINGS.find((listing) => listing.name === thread.unit)).filter(Boolean);
}
...
export default function TourDetail() {
  ...
  const listings = useMemo(() => getDefaultListings(routeShortlistedIds), [routeShortlistedIds]);
  const totalListings = listings.length;
  const groupedListings = useMemo(() => {
    const groups = new Map();

    listings.forEach((listing) => {
      const thread = THREADS.find((item) => item.unit === listing.name);
      const enriched = {
        ...listing,
        thread,
      };

      if (!groups.has(listing.area)) {
        groups.set(listing.area, []);
      }
      groups.get(listing.area).push(enriched);
    });
  ...
  const buyerName = buyer.name || 'Chen family';
  const buyerAvatar = buyer.avatar || getAvatarFromName(buyerName);
  const buyerSubtitle = buyer.subtitle || '3BR rental · move-in June · pet-friendly';
