...
import { AB, LISTINGS, TOURS } from '../data';
...
function getShortlist(shortlistedIds = []) {
...
export default function BuyerView() {
  ...
  const shortlist = useMemo(() => getShortlist(routeShortlistedIds), [routeShortlistedIds]);
  const hasExistingClients = TOURS.length > 0;

  const [step, setStep] = useState('mode');
  const [buyerMode, setBuyerMode] = useState(() => (hasExistingClients ? 'existing' : 'new'));
  const [selectedBuyerId, setSelectedBuyerId] = useState(() => (TOURS[0]?.id || ''));
  ...
  useEffect(() => {
    ...
    if (hasExistingClients && !selectedBuyerId) {
      setSelectedBuyerId(TOURS[0].id);
    }
  }, [buyerMode, hasExistingClients, selectedBuyerId]);

  const selectedTour = TOURS.find((tour) => tour.id === selectedBuyerId) || null;
  const activeBuyer = buyerMode === 'existing'
    ? {
      name: selectedTour?.buyer || 'Client',
      avatar: selectedTour?.avatar || 'C',
      subtitle: selectedTour?.type || 'Client preview',
      helper: selectedTour?.preview || 'Shortlist ready to share',
    }
    : {
  ...
  const canAdvance = buyerMode === 'existing' ? Boolean(selectedTour) : Boolean(buyerName.trim());
  const canSend = canAdvance;
  const shareTargetName = buyerMode === 'existing'
    ? selectedTour?.buyer || 'this client'
    : buyerName.trim() || 'this client';
  const shareButtonLabel = buyerMode === 'existing'
    ? `Send to ${selectedTour?.buyer || 'client'}`
    : buyerName.trim()
        ...
        {step === 'mode' && buyerMode === 'existing' && (
          <div style={{ marginTop: 16, background: AB.white, borderRadius: 20, border: `1px solid ${AB.border}`, overflow: 'hidden' }}>
            ...
            {TOURS.map((tour, index) => {
              const active = selectedBuyerId === tour.id;
              return (
                <button
                  key={tour.id}
                  onClick={() => setSelectedBuyerId(tour.id)}
                  style={{
                    width: '100%',
                    border: 0,
                ...
                >
                  <Avatar label={tour.avatar} color={tour.color} size={42} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 600, fontSize: 14.5 }}>{tour.buyer}</div>
                    <div style={{ fontSize: 12.5, color: AB.gray, marginTop: 2 }}>{tour.type}</div>
        ...
        {step === 'details' && (
          <div style={{ marginTop: 16, background: AB.white, borderRadius: 20, border: `1px solid ${AB.border}`, overflow: 'hidden' }}>
            <div style={{ padding: '10px 14px', borderBottom: `1px solid ${AB.border}`, background: AB.bg, display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: AB.gray, fontFamily: 'ui-monospace, monospace' }}>
              ...
              butler.ai/client-preview/{buyerMode === 'existing' ? selectedTour?.id || 'draft' : buyerName.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-') || 'draft'}
            </div>
